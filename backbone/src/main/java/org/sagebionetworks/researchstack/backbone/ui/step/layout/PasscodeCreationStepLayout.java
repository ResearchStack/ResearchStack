...
import org.sagebionetworks.researchstack.backbone.ui.views.PinCodeLayout;
import org.sagebionetworks.researchstack.backbone.utils.ThemeUtils;

public class PasscodeCreationStepLayout extends PinCodeLayout implements StepLayout
{
    public static final String RESULT_OLD_PIN = "PassCodeCreationStep.oldPin";

    protected StepCallbacks        callbacks;
    protected PasscodeStep step;
    ...
    public PasscodeCreationStepLayout(Context context)
    {
        super(context);
    }

    public PasscodeCreationStepLayout(Context context, AttributeSet attrs)
    {
        super(context, attrs);
    }

    public PasscodeCreationStepLayout(Context context, AttributeSet attrs, int defStyleAttr)
    {
                        ...
                        if(pin.equals(currentPin))
                        {
                            new Handler().postDelayed(() -> {
                                ...
                                if (state == State.CONFIRM) {
                                    StorageAccess.getInstance().createPinCode(getContext(), pin);
                                }
                            }, 300);
                        }
                        else
