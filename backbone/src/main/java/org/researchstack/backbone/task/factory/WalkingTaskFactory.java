...
public class WalkingTaskFactory {
...

        {
            ...
            if (restDuration > 0) {
                if (restDuration > 0) {
                    ...
                    String title = String.format(titleFormat, convertDurationToString(restDuration));
                    step.setTitle(title);
     ...
     */
    private static String convertDurationToString(int durationInSeconds) {
        try {
            long now = System.currentTimeMillis();
            long then = now + (durationInSeconds * DateUtils.SECOND_IN_MILLIS);
            return DateUtils.getRelativeTimeSpanString(then, now, DateUtils.SECOND_IN_MILLIS).toString().toLowerCase();
        } catch (RuntimeException e) {
            // this is purely to catch the unit test runtime exception here
            // DateUtils is not a mock-able static class
            return "";
        }
    }
