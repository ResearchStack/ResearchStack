...
public class StudyLandingLayout extends ScrollView
{
    ...
    {
        logoView.setImageResource(TouchKitApplication.getInstance().getLargeLogoDiseaseIcon());
        ...
        emailConsent.setOnClickListener(v -> {
            ...
            //TODO Clean this up, should no write/read on main thread
            File consentFile = getConsentFormFileFromExternalStorage();
            String appName = getResources().getString(TouchKitApplication.getInstance().getAppName());
    ...
    private File getConsentFormFileFromExternalStorage()
    {
        ...
        String basepath = extStorageDirectory + "/" + TouchKitApplication.getInstance()
                .getExternalSDAppFolder();

        int fileResId = TouchKitApplication.getInstance().getConsentForm();
